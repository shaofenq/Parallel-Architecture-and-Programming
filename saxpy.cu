...
saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    // allocate for x, y and res --> return *ptr
    ...
    // record time after data is tranformed
    ...
    //
    cudaMemcpy(resultarray,device_result, N*sizeof(float), cudaMemcpyDeviceToHost); //Device to Host

    // end timing after result has been copied back into host memory
    double endTime = CycleTimer::currentSeconds();

    // record kernel(GPU) run time
    ...
    printf("Kernel: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * kernel_runTime, toBW(totalBytes, kernel_runTime));
    
    double overallDuration = endTime - startTime;

    // show both total duration and kernel/GPU run time
